...
The [repository](https://github.com/applanga/github-workflow-example) contains 2 sample workflows. Running and past workflow actions can be tracked under the [**Actions** Tab](https://github.com/applanga/github-workflow-example/actions).
...
The preceding workflow configuration is setup to run on any given branch. For this to work though the configuration file has to first be created from the default branch. In other words start first by adding the workflow file to the `master` branch of the repository, which allows github to pick up the workflow. Next you can create a branch of the master branch and update the workflow file as needed. To run this workflow an api request should be made to Github specifying either the file name(`applanga-pull.yml`) or workflow id. More on this in the [Configure Webhook Endpoint](#configure-webhook-endpoint) section.
